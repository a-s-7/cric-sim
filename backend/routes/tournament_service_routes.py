...
def get_tournament(tournament_base_id):
    return jsonify(ts.get_tournament_info(tournament_base_id))
...
def get_tournament_stages(tournament_id):
    only_active_stages = request.args.get("onlyActiveStages") == "true"
    
    return jsonify(ts.get_tournament_stages(tournament_id, only_active_stages))
