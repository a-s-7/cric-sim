...
function PointsTable({ pointsTableData, headerColor, topQualifiers }) {
    ...
    const calculateNRR = (rS, bF, rC, bB) => {
        const totalOversFaced = bF / 6;
        const totalOversBowled = bB / 6;

        const runRate = totalOversFaced > 0 ? rS / totalOversFaced : 0;
        const runRateConceded = totalOversBowled > 0 ? rC / totalOversBowled : 0;

        return runRate - runRateConceded;
    }
            ...
            <tbody className="font-['Nunito_Sans']">
                {pointsTableData.map((team, index) => (
                    <tr key={team.name} className={`${index < topQualifiers ? 'bg-gray-100' : ''} hover:bg-[#e4e4e4] transition-colors duration-200 text-sm`}>
                        ...
                        <td className="text-center py-3 px-2 border-b border-zinc-200">{calculateNRR(team.runsScored, team.ballsFaced, team.runsConceded, team.ballsBowled)}</td>
